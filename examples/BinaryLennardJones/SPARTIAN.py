...
def spartian(config, subdomain, atoms, molecules):
    ...
    LJ = pyMRMD.action.LJ_IdealGas(config['LJ']['capping'], config['LJ']['cutoff'], config['LJ']['sigma'],
                                   config['LJ']['epsilon'],
                                   2, True)
    ...
    thermostat = pyMRMD.action.LangevinThermostat(config["temperature_relaxation_coefficient"],
    ...
    weighting_function = pyMRMD.weighting_function.Slab(config["center"], config["atomistic_region_diameter"],
                                                        config["hybrid_region_diameter"], config["lambda_exponent"])

    h_atoms = pyMRMD.data.HostAtoms(atoms)
    rhoA = (h_atoms.get_type_np() == 0).sum() / volume
    rhoB = (h_atoms.get_type_np() == 1).sum() / volume

    thermodynamic_force = pyMRMD.action.ThermodynamicForce([rhoA, rhoB], subdomain, config["density_bin_width"],
    ...
    for step in range(config['time_steps']):
        ...
        if (step > config["density_start"]) and (step % config['density_sampling_interval'] == 0):
            thermodynamic_force.sample(atoms)

        if (step > config["density_start"]) and (step % config['density_update_interval'] == 0):
            density_profile = pyMRMD.analysis.get_axial_density_profile(atoms.num_local_atoms,
                                                                        atoms,
                                                                        2,
            ...
            density_profile.scale(
                1 / (density_profile.binSize * subdomain.diameter[1] * subdomain.diameter[2]))
            Xrho1 = pyMRMD.analysis.get_fluctuation(density_profile, rhoA, 0)
            Xrho2 = pyMRMD.analysis.get_fluctuation(density_profile, rhoB, 1)

            thermodynamic_force.update(config["smoothing_sigma"],
                                       config["smoothing_intensity"])
