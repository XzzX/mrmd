...
def npt(config, subdomain, atoms):
    ...
    LJ = pyMRMD.action.LennardJones(config['LJ']['capping'], config['LJ']['cutoff'], config['LJ']['sigma'],
                                    config['LJ']['epsilon'],
                                    2, True)
    ...
    for step in range(config['time_steps']):
        ...
        if (step > config['barostat_start']) and (step % config['barostat_interval'] == 0):
            pyMRMD.action.berendsen_barostat.apply(atoms,
